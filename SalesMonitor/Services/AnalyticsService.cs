...
namespace SalesMonitor.Services
{
    public class AnalyticsService
    {
        ...
        public async Task<List<AbcResult>> GetAbcAnalysisAsync(DateTime? from = null, DateTime? to = null)
        {
            ...
            var productRevenue = sales
                ...
                .Select(g => new { g.Key.ProductId, g.Key.Name, Revenue = g.Sum(s => s.TotalAmount) })
            ...
            {
                var share = totalRevenue > 0 ? (double)(item.Revenue / totalRevenue) * 100 : 0;
                cumulative += share;
        ...
        {
            var sales = await _db.Sales.Include(s => s.Product).ToListAsync();
            var results = new List<XyzResult>();

            foreach (var group in sales.GroupBy(s => new { s.ProductId, s.Product.Name }))
            {
        ...
        public async Task<List<PlanFactResult>> GetPlanFactAsync(DateTime from, DateTime to)
        {
            ...
            foreach (var product in products)
            {
                var fact = await _db.Sales
                    .Where(s => s.ProductId == product.Id && s.SaleDate >= from && s.SaleDate <= to)
                    .SumAsync(s => s.TotalAmount);

                var daysInPeriod = (to - from).Days + 1;
                var planForPeriod = product.PlanSales * daysInPeriod / 30;
                var deviation = fact - planForPeriod;
                var deviationPercent = planForPeriod != 0 ? (double)(deviation / planForPeriod) * 100 : 0;
        ...
        public async Task<List<DeviationResult>> GetDeviationsAsync(DateTime from, DateTime to)
        {
            ...
            return sales.Select(s => new DeviationResult
            {
                SaleId = s.Id,
    ...
    public static class StatisticsHelper
    {
        public static double CoefficientOfVariation(IEnumerable<double> values)
        {
            ...
            var mean = data.Mean();
            return mean != 0 ? (data.StandardDeviation() / mean) * 100 : 0;
        }
    }
}